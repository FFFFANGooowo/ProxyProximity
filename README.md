...
- **代理Google AI API**：
  ...
  - **注意**：如果您使用的是支持多种格式的AI终端（如Cherry Studio），请确保切换到正确的API格式或使用专门支持Google API的客户端，以避免兼容性问题。
...
- **代理Anthropic API**：
  ...
  - **注意**：如果您使用的是支持多种格式的AI终端（如Cherry Studio），请确保切换到正确的API格式或使用专门支持Anthropic API的客户端，以避免兼容性问题。
- *Anthropic格式的代理是实验性的，没有经过验证。可能会遇到兼容性问题（如无法上传图像或函数调用Tool list）或无法使用*
